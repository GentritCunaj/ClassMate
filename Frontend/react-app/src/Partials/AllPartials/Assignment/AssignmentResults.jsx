...
const AssignmentResults = () => {
   ...
   // Funksioni për shkarkimin e file-it
  const handleDownload = async (submissionId, fileName) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`https://localhost:7168/Submission/DownloadSubmission/${submissionId}`, {
        headers: {
          Authorization: 'Bearer ' + token
        },
        responseType: 'blob' // Përcaktoje këtu responseType në 'blob'
      });

      // Krijimi i një URL nga objekti blob
      const url = window.URL.createObjectURL(new Blob([response.data]));
      
      // Krijimi i elementit a për shkarkimin e file-it
      ...
      // Lirimi e burimit të URL pas shkarkimit
                    ...
                    <table className="results-table">
                        <thead>
                            <tr>
                                ...
                                <th>Submitted On</th>
                                <th>File Name</th>
                                <th>Actions</th> {/* Add column for download button */}
                            </tr>
                        </thead>
                        <tbody>
